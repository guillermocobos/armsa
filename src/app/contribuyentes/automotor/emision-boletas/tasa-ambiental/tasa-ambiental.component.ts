...
export class TasaAmbientalComponent implements OnInit, OnDestroy {
  ...
  imprimirBoleta(deuda: any, indice: number) {
    ...
    const body = {
      ...
      importe: deuda.subtotal,
      totalPagar: deuda.subtotal,
      modelo: this.estadoDeuda.login.modelo,
      zona: this.estadoDeuda.login.modelo,
      leyenda: deuda.importeLetras,
    ...
    this.imprimirSubscription = this.automotoresService.imprimirTasaAmbiental(body)
      .subscribe((response: any) => {
        this.loadingImprimirBoleta[indice] = false;
        const blob = new Blob([response], {
          type: 'application/pdf'
        });
        FileSaver.saveAs(blob, `F935_tasa_ambiental_${body.dominio + '_' + deuda.numeroCuota + '_' +  deuda.anio.toString().trim()}.pdf`);
      });
  }
