...
@Component({
  selector: 'app-planes-pago-inmobiliario-estado-deuda',
...
export class PlanesPagoComponent implements OnInit, OnDestroy {
  ...
  calcularDeuda() {
    ...
    if (this.formRecalculo.valid) {
      ...
      this.planesPagoSubscription = this.inmobiliarioService.planesPago(body)
        .subscribe((data) => {
          console.log(data);
          this.planesPago.estadoPlanAutos = data;
  ...
  //   this.imprimirSubscription = this.inmobiliarioService.imprimirHistorialMultas(body)
  //     .subscribe((response: any) => {
  //       this.loading = false;
  //       const blob = new Blob([response], {
  //         type: 'application/pdf'
  //       });
  //       FileSaver.saveAs(blob, `historial_multas_${this.form.get('catastro').value}.pdf`);
  ...
  imprimirBoleta(deuda: any, indice: number) {
    ...
    this.imprimirSubscription = this.inmobiliarioService.imprimirBoletaInmobiliario(body)
      .subscribe((response: any) => {
        ...
        FileSaver.saveAs(blob, `F981_plan_de_pago_inmobiliario_${this.datosDeConsulta.catastro + '_' + deuda.numeroCuota + '_' + deuda.anio}.pdf`);
